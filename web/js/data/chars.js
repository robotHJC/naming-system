...
(function (global) {
   ...
   * 两种笔画，两个用途 —— 不要混用
   * ---------------------------------------------------------------------
   *   strokes   康熙笔画 —— 三才五格、姓名卦专用。
   *             五格剖象法与梅花易数**定义在康熙笔画上**，
   *             换成简体，数理连定义都不成立。
   *             来源：内置 490 字是人工核定的；联网加入的字是按部首
   *             推算的近似值（见 infer.js 的 estimateKangxi，实测规则
   *             准确率只有 93.3%，所以必须带推断标记）。
   *
   *   strokesSC 简体笔画 —— 字形均衡（可读性）、书写难度、「简单」用。
   *             人写的是简体：「听」是 7 画，不是康熙「聽」的 22 画。
   *             来源：新华字典的 strokes 字段，可靠。
   *
   * 内置 490 字在联网同步《新华字典》后会自动回填 strokesSC
   * （见 lexicon.js 的 applySimplifiedStrokes）。拿不到时退回康熙笔画 ——
   * 简繁多数字形一致，误差有限，但总比 0 好。
   * ===================================================================== */
  NS.strokesSC = function (c) {
    if (!c) return 0;
    return c.strokesSC > 0 ? c.strokesSC : (c.strokes || 0);
  };

  /** 该字的康熙笔画是否只是推断值（联网加入、未经人工核定） */
  NS.kangxiIsInferred = function (c) {
    return !!(c && c.__inferred && c.__inferred.strokesConfidence);
  };
